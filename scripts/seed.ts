...
const SEED_USER_ID = "user_2iH1HfymzvFxTHobspZGKmT9TPk";
...
const SEED_ACCOUNTS = [
  ...
  { id: "account_2", name: "Checking", userId: SEED_USER_ID, plaidId: null },
];
...
const generateTransactionsForDay = (day: Date) => {
  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    const isExpense = Math.random() > 0.6; // 60% chance of being a expense
    const amount = generateRandomAmount(category);
    const formattedAmount = convertAmountToMiliunits(isExpense ? -amount : amount);

    SEED_TRANSACTIONS.push({
      id: `transaciton_${format(day, "yyyy-MM-dd")}_${i}`,
